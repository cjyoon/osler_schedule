...
# setwd("~/Dropbox/Osler/Osler_shiny_JARSAR")
# file_list <- list.files('2024/', pattern = "\\.tsv$", full.names = TRUE)
# 
...
# #
# #
# write_delim(new_table, file='~/Dropbox/Osler/Osler_shiny_JARSAR/schedule/osler_schedule_table.tsv', delim='\t')
new_table = read_delim('schedule/osler_schedule_table.tsv', delim='\t')
block_table = read_delim('schedule/block_view.tsv', delim='\t')

last_updated_time = file.info("Resident Schedule 2024-2025.xlsx")$mtime
...
ui <- fluidPage(
  ...
  tags$div(
    ...
    tags$h1(" Osler JAR/SAR Schedule: No longer updated as of 03/2025. Refer to the combined schedule for PGY-1,2,3 at bit.ly/osler1", style = "margin-right: 15px;"),  # Title with some margin
    
  ),
  tags$title("Osler JAR/SAR Schedule"), 
  ...
  fluidRow(
    column(2, checkboxInput("toggle_table", "Block schedule", value = FALSE)),  # Toggle switch
    ...
    column(2, class = "col-xs-2", style = "padding-left:0px;", 
           ...
           selectInput("rotation", "Rotation:",
                         choices = c("","Supervisor", "Janeway", "Barker", "Longcope", "Thayer", "Brancati", "NightWatch", "Relief","NATO", "Jeopardy", "MICU", "CCU", "Triage", "MPC","BMICU", 'BCCU', "CJ", "Liver", "Polk", "Ambulatory", "MTL", "Solids", "Leuks", "GenCards", 'Cardiomyopathy', "FirmJAR", "Research", "Elective", 'Peds', "Women'sHealth", "AddictionMedicine", "HIV/HCV", "Geri"), 
                       selected = NULL, multiple=F)
    ),
    column(2, class = "col-xs-2", style = "padding-left:0px;", 
           # Filter by call schedule
           selectInput("callSchedule", "Call Schedule:",
                       choices = c("", "OLDS", "NEWS","WOLF", "CALL", "POST", "OFF", "DAY", "GOOD", "NIGHTS", "BOOK","JEOPARDY", '-'), 
                       selected = NULL, multiple=F)
    )
  ),
  fluidRow(
...
server <- function(input, output, session) {
  ...
  output$last_updated <- renderText({
    paste("Last updated:", format(last_updated_time, "%Y-%m-%d %H:%M:%S %Z"))
  })
  ...
  output$table <- renderDT({
    if(input$toggle_table){ # if want to view in block view
      ...
      if (!is.null(rotation) && rotation != "") {
        # Use str_detect to filter for shifts that contain the selected schedule pattern
        new_table <- new_table %>%
          filter_at(vars(ends_with(selected_start_col_name)), any_vars(str_detect(., rotation)))
      }
